...
namespace Dapper
{
    public static partial class DapperExt
    {
        ...
        public static Task<IEnumerable<T>> GetListPagedAsync<T>(this IDbConnection connection, int pageNumber, int rowsPerPage, string conditions, string orderby, object parameters = null, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            query = query.Replace("{StartRow}", pageNumber.ToString());
            query = query.Replace("{RowsPerPage}", rowsPerPage.ToString());
            query = query.Replace("{WhereClause}", conditions);
            query = query.Replace("{OrderBy}", orderby);

            //(página - 1) * número_de_itens_por_página
            var offSetNumber = (pageNumber <= 0 ? 0 : pageNumber - 1);

            query = query.Replace("{Offset}", (offSetNumber * rowsPerPage).ToString());
